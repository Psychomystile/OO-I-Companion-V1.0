...
function campaignView(){
 ...
 <div class="campaign-nav">${['hangar','inventory','market','history'].map(v=>`<button data-c-action="tab" data-value="${v}" class="${campaignTab===v?'primary':''}">${{hangar:'Hangar',inventory:'Inventory',market:'Iron Market',history:'History & settings'}[v]}</button>`).join('')}</div>
 ${campaignTab==='inventory'?inventoryView():campaignTab==='market'?marketView():campaignTab==='history'?historyView():hangarView()}`;
}
