...
function baseBuildView(){
 ...
 </aside>${modePanel()}<div class="build-layout build-stacked"><section class="panel"><span class="eyebrow">01 / FACTION AND RIG</span><div class="factions">${FACTIONS.filter(([id])=>id!=='nomads').map(([id,n])=>`<button data-action="faction" data-value="${id}" aria-pressed="${id===state.faction}" class="${id===state.faction?'selected':''}">${n}</button>`).join('')}</div>
...
function pregamePanel(){const p=pregameSettings(),locked=pregameLocked();return `${engagementSelector()}<section class="panel pregame-panel"><h3>Battlefield setup</h3><div class="form-grid">${[
 ['deployment','Deployment',Array.from({length:12},(_,i)=>[i+1,'Deployment '+(i+1)]),'D12'],
 ['density','Terrain density',[[0,'Low · 10%'],[1,'Medium · 30%'],[2,'High · 50%']],'D6'],
 ['terrain','Terrain type',TERRAINS.map((t,i)=>[i,t[0]]),'D12']
 ].map(([key,label,values,die])=>`<div><label class="field"><span>${label}</span><select data-pregame="${key}" ${locked?'disabled':''}>${options(values,p[key])}</select></label><button data-pregame-random="${key}" ${locked?'disabled':''}>Random · ${die}</button></div>`).join('')}</div><p class="small">Agree the setup with your opponent. Engagement-specific deployment and terrain instructions take priority.${isCampaign()?' The selected terrain carries into Aftermath.':' Terrain rewards apply only in campaigns.'}</p><details><summary>Deployment maps · Rulebook pp. 93–94</summary><p class="small">Selected: Deployment ${p.deployment}. Deployment 10 is for more than two players.</p><img class="deployment-map" src="images/pregame/deployments-${p.deployment<=6?'096':'097'}.jpg" alt="Deployment maps ${p.deployment<=6?'1 to 6':'7 to 12'}"></details></section>`;}
...
function weaponOptions(rig,selected,slot=null,currency='Iron',placeholder=true){
 ...
 const list=rig.weapons.flatMap((w,i)=>w.group===group&&(slot!==2||isHull(w))&&(rig.weight!=='Colossal'||isHull(w)===hull)?[[i,`${w.name} · ${w.iron} ${currency}`]]:[]);
...
function handleClick(ev){
 ...
 const targetRig=state.squad.find(r=>r.id===(d.id||d.armLoss||d.rupturePicker));if(targetRig&&rigUnusable(targetRig)&&d.mobile!=='select'&&d.mobile!=='undo')return;
