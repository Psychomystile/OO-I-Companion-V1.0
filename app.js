...
function configErrors(r){
 ...
 r.weapons.forEach((w,i)=>{if(w===null)return; if(!rig.weapons[w])errors.push('Invalid weapon.');else if(i===2&&(rig.weight!=='Colossal'||!isHull(rig.weapons[w])))errors.push('The Hull slot requires a Colossal Rig and a weapon with the Hull perk.');});
...
let state={version:1,mode:'standard',name:'My squadron',faction:'krim',formation:0,squad:[],round:1,view:'build',saved:[]};
...
function baseBuildView(){
 ...
 <div class="stack"><button class="primary" data-action="confirm" ${!state.squad.length||squadronErrors().length?'disabled':''}>${isCampaign()?'Review and start campaign':'Confirm and open battle sheets'}</button><button data-action="save" ${!state.squad.length?'disabled':''}>Save named squadron</button><button data-action="new">New squadron</button></div>
...
function toggleActivation(r){
 const b=r.battle;if(b.exploded||b.shutDown)return;if(b.skipActivation){notify('This Rig must skip this activation after Emergency Shutdown.');return;}b.activated=!b.activated;
...
function battleCard(r){const rig=battleProfile(r),b=r.battle,destroyed=b.parts.filter(x=>x==='disabled').length>=2;
 return `<article class="panel battle-card ${activeRigId()===r.id?'selected-rig':''} ${b.activated?'activated':''} ${destroyed?'destroyed':''} ${b.exploded?'rig-exploded':''}" data-card="${r.id}">${rigBanner(r)}<fieldset class="rig-controls" ${b.exploded?'disabled':''}>${statusButtons(r)}<div class="card-heading"><div><span class="eyebrow">${rig.name.toUpperCase()} · ${WEIGHTS[rig.weight].toUpperCase()}${rig.faction==='nomads'?' · NOMAD':''}</span></div><div class="row activation-controls"><button class="power-button ${b.shutDown?'power-on':''}" data-shutdown="${r.id}" title="Shut Down: forfeit all Actions; cool to 0 unless engine damage prevents it" aria-label="Shut Down" ${b.activated||b.actions||actionLimit(r)===0?'disabled':''}><svg viewBox="0 0 32 32" aria-hidden="true"><path d="M16 3v12M8 7a12 12 0 1 0 16 0" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round"/></svg><span>Shut Down</span></button><button class="activation-marker" data-action="activated" data-id="${r.id}" aria-pressed="${b.activated}">${b.exploded?'EXPLODED':b.shutDown?'Shut down':b.activated?'✓ Activated':'Not activated'}</button></div></div>
 ${pilotBattleHeader(r)}
 ${destroyed&&!b.exploded?`<p class="warning danger">DESTROYED · resolve explosion check (p. 33).</p><button class="danger" data-exploded="${r.id}">Mark exploded</button>`:''}
 ...
 <div class="track-label"><span>Actions used <span class="small">/ ${actionLimit(r)} available Actions</span></span>${counter(r.id,'actions',b.actions,actionLimit(r))}</div>${actionHistoryView(r)}
 ...
 <details><summary>Notes & preparations</summary><label class="field"><span>Preparations, permanent effects, targets</span><textarea data-field="notes" data-id="${r.id}" maxlength="2000" placeholder="E.g. Brace prepared; skip the next activation…">${esc(b.notes)}</textarea></label></details></fieldset>${b.exploded?'<p class="exploded-label">EXPLODED · RIG UNUSABLE</p><button data-mobile="undo">↶ Undo last change</button>':''}</article>`;
}
...
function baseBattleView(){return `<div class="toolbar"><div><span class="eyebrow">${esc(state.name)}</span><h2>Round ${state.round} · ${currentEngagement()[1]}</h2><p class="small">${engagementDuration(currentEngagement())}${state.engagementEnded?' · ENGAGEMENT COMPLETE — finish the game / open Aftermath':''}</p><p class="small">${deployedRigs().filter(r=>r.battle.activated).length}/${deployedRigs().length} Rigs activated · toggle each Rig to correct the count.</p></div><div class="row">${!isCampaign()?`<button data-action="finish-normal" ${!state.squad.length?'disabled':''}>Finish game</button>`:''}<button class="primary" data-action="next-round" ${!state.squad.length?'disabled':''}>Maintenance phase</button></div></div>
 ${maintenanceView()}${battleNavigation()}${state.squad.length?`<div class="battle-grid ${battleFocus?'focus-rig':''}">${deployedRigs().map(battleCard).join('')}</div>`:'<div class="empty"><strong>No battle sheets</strong>Build and confirm a squadron to begin.</div>'}
...
function baseHandleClick(ev){
 ...
 if(action==='confirm'){if(editing){notify('Save or cancel your Rig changes.');return;}if(!state.squad.length||squadronErrors().length)return;state.view='battle';}
 if(action==='save'){
...
function setCampaignPath(path,value){const parts=path.split('.');if(parts.some(p=>['__proto__','constructor','prototype'].includes(p)))return;let obj=state.campaign;for(const p of parts.slice(0,-1)){if(!(p in obj))return;obj=obj[p];}obj[parts.at(-1)]=value;}
...
function newReport(){
 ...
 c.report={result:'win',terrain:0,title:`Engagement ${c.games.length+1}`,outcomeDie:null,normalVP:0,salvageObjectives:0,scenarioSalvage:0,scenarioReason:'',cityPilot:'',settlementReserves:true,environment:[{rig:'',part:0},{rig:'',part:0}],underdog:'none',underdogDie:null,opponentValue:0,rows:{}};
 ...
 c.phase='report';state.view='campaign';
}
...
function applyReport(){const c=state.campaign,rep=c.report;if(!rep||c.phase!=='report')throw Error('No pending Aftermath.');const p=outcomePreview(rep);
 ...
 c.report=null;c.market=null;c.phase='aftermath';c.deployed=c.deployed.filter(id=>state.squad.some(r=>r.id===id));campaignTab='hangar';
}
function reportView(){
 ...
 <section class="panel" style="margin-top:18px"><h3>3. Underdog and scenario adjustments</h3><p class="small">Current roster value: ${rosterValue()} Salvage (recruit costs plus banked Salvage). Compare values after rewards with your opponent. Select the applicable reinforcement: D3 for lower value, or D6 when unable to field the agreed formation (p. 106). Record any group ruling as a scenario adjustment.</p><div class="form-grid">${numberInput('Opponent roster value (reference)','report.opponentValue',rep.opponentValue)}${selectField('Underdog reinforcement','report.underdog',[['none','None'],['lower','Lower roster value · D3'],['short','Unable to field formation · D6']],rep.underdog)}${rep.underdog!=='none'?dieInput('Underdog Salvage','report.underdogDie',rep.underdogDie,rep.underdog==='lower'?3:6):''}${numberInput('Scenario Salvage adjustment','report.scenarioSalvage',rep.scenarioSalvage,-999,999)}<label class="field"><span>Scenario adjustment / mission notes</span><input data-c-field="report.scenarioReason" maxlength="500" value="${esc(rep.scenarioReason)}"></label></div></section>
...
function hangarView(){const c=state.campaign,locked=c.phase==='battle';return `${engagementSelector()}<section class="panel"><div class="section-top"><h3>Deployment</h3><span class="badge">${deployedRigs().length} selected / ${state.squad.length} owned</span></div>${locked?'<p class="warning">An Engagement is in progress. Roster and market changes are locked until Aftermath.</p><div class="row"><button class="primary" data-c-action="resume">Resume battle sheets</button><button data-c-action="finish-battle">Finish Engagement / Aftermath</button></div>':`<div class="form-grid"><label class="field"><span>Agreed formation</span><select data-field="formation">${options(FORMATIONS.flatMap((f,i)=>i?[[i,f[0]]]:[]),state.formation)}</select></label><details id="deployment-options"><summary>Short-handed deployment${c.shortHanded?' · enabled':''}</summary>${checkField('Unable to fill formation: deploy remaining available Rigs','shortHanded',c.shortHanded)}</details></div><p class="small">Choose Rigs below; the rest remain in reserve. A mercenary uses its separate support slot.</p>${squadronErrors().length?`<p class="warning">${squadronErrors().map(esc).join('<br>')}</p>`:''}<button class="primary" data-c-action="deploy" ${squadronErrors().length?'disabled':''}>Begin next Engagement</button>`}</section>
 ...
 <details id="hangar-management-${r.id}"><summary>Rig & pilot management</summary><label class="field"><span>Rig name</span><input data-c-rig-name="${r.id}" maxlength="45" value="${esc(r.name)}"></label><select id="pilot-${r.id}" aria-label="Assign Ironclad">${options([['rookie','Recruit Rookie · 0 Salvage'],['experienced','Recruit Experienced · 1 Salvage'],...c.pilots.map(p=>[p.id,`${p.name} · ${PILOTS[p.grade].name} · ${p.xp} XP`])],'rookie')}</select><button data-c-action="pilot" data-id="${r.id}">Assign selected Ironclad</button><p class="small">The current Ironclad moves to the unassigned pilot pool. XP, level and re-rolls stay with the Ironclad.</p><button class="danger" data-c-action="scrap-rig" data-id="${r.id}">Scrap chassis · ${Math.ceil(RECRUIT_COST[rig.weight]/2)} Salvage</button></details>`:''}</article>`;}).join('')}</div>${!state.squad.length?'<div class="empty">No Rigs remain. Visit the Iron Market or claim rewards to rebuild.</div>':''}`;}
...
function buyWeapon(){checkMarket();const c=state.campaign,m=c.market,r=currentMarketRig(),slot=Number(m.slot);let index=Number(m.weapon),traits=[];
 ...
 const w=rigOf(r).weapons[index];if(!w)throw Error('Select a weapon.');if(slot===2&&(rigOf(r).weight!=='Colossal'||!isHull(w)))throw Error('The Hull slot requires a Hull weapon on a Colossal Rig.');
...
function weaponOptions(rig,selected,slot=null,currency='Iron',placeholder=true){
 ...
 const list=rig.weapons.flatMap((w,i)=>w.group===group&&(slot!==2||isHull(w))?[[i,`${w.name} · ${w.iron} ${currency}`]]:[]);
 return list.length?`<optgroup label="${group==='common'?'Common weapons · '+rig.weight+' · p. 48':group==='nomad'?'Nomad weapons · '+rig.name:'Faction weapons · '+rig.name}">${options(list,selected)}</optgroup>`:'';
 }).join('');
}
function rigCatalog(available){
 return [['Faction Rigs',available.filter(r=>r.faction!=='nomads')],['Nomad Rigs',available.filter(r=>r.faction==='nomads')]].filter(([,rigs])=>rigs.length).map(([label,rigs])=>`<section class="rig-catalog-group"><div class="section-top"><h3>${label}</h3>${label==='Nomad Rigs'?`<button data-action="faction" data-value="nomads" class="small" aria-pressed="${state.faction==='nomads'}">Nomad-only squadron</button>`:''}</div>${label==='Nomad Rigs'?'<p class="small">Independent chassis, available as support in standard squadrons.</p>':''}<div class="rig-grid">${rigs.map(r=>`<button class="rig-choice ${selected===r.id?'selected':''}" data-action="rig" data-value="${r.id}"><span class="rig-mark">${r.iron} IRON</span><strong>${r.name}</strong><span class="small">${WEIGHTS[r.weight]} · SPD ${r.speed} · EXH ${r.exhaust}</span></button>`).join('')}</div></section>`).join('');
}
function weaponMarketField(r,value){return `<label class="field"><span>Weapon</span><select data-c-field="market.weapon">${weaponOptions(rigOf(r),value,null,'Salvage',false)}</select></label>`;}
...
function handleMenuAction(el){const a=el.dataset.menuAction;
 ...
 if(['build','resume'].includes(a)){
  ...
  else if(state.mode==='standard'&&state.squad.length&&!squadronErrors().length)state.view='battle';
  render();
 }
}
function switchWorkspace(id,mode='campaign',imported=null){
 ...
 menuScreen='play';state=next;workspaces=hub;campaignTab=entry?.campaignTab||'hangar';ensureCampaign();resetDraft();
...
function handleClick(ev){
 ...
 const targetRig=state.squad.find(r=>r.id===(d.id||d.armLoss||d.rupturePicker));if(targetRig?.battle.exploded&&d.mobile!=='select'&&d.mobile!=='undo')return;
...
function handleChange(ev){
 if(state.view==='battle'&&state.squad.find(r=>r.id===ev.target.dataset.id)?.battle.exploded)return;
